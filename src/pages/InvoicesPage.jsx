...
import { formatINR, todayISO, formatISTDate, normalizeMobile } from "../utils/appUtils";
import { buildWhatsAppUrl } from "../utils/whatsappUtils";
export function InvoicesPage({ invoices, invoiceItems, invoicePayments = [], businessSettings, customers = [], bookings = [], onUpdated }) {
  const [paymentInvoiceId, setPaymentInvoiceId] = useState(null);
  const [detailInvoiceId, setDetailInvoiceId] = useState(null);
  const [editInvoiceData, setEditInvoiceData] = useState(null);
  const [editInvoiceForm, setEditInvoiceForm] = useState({ total_amount: 0, paid_amount: 0, due_amount: 0, discount: 0, payment_status: "", payment_method: "", cash_amount: 0, upi_amount: 0, warranty_terms: "", customer_address: "" });
  ...
  function buildInvoicePdf(inv, items, index) {
    ...
    if (inv.is_gst_invoice && business.gst_number) addLine(`GSTIN: ${business.gst_number}`);
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    addLine(`Invoice No: ${invoiceNo}`, `Date: ${inv.created_at ? new Date(inv.created_at).toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" }) : ""}`);
    doc.setFont("helvetica", "normal");
    const customerObj = customers.find((c) => normalizeMobile(c.mobile) === normalizeMobile(inv.mobile));
    const bookingObj = bookings.find((b) => String(b.id) === String(inv.booking_id));
    const customerAddress = inv.customer_address || bookingObj?.address || customerObj?.address || "";
    addLine(`Customer: ${inv.customer_name || ""}`, `Mobile: ${inv.mobile || ""}`);
    if (customerAddress) {
      addLine(`Address: ${customerAddress}`);
    }
    ...
    if (inv.warranty_terms) {
      y += 8;
      doc.setFont("helvetica", "bold");
      addLine("Warranty & Coverage Terms");
      doc.setFont("helvetica", "normal");
      const splitTerms = doc.splitTextToSize(inv.warranty_terms, pageWidth - margin * 2);
    ...
    if (business.signature_url) {
      addLine("Authorized Signatory (Digitally Signed)");
    } else {
      addLine("Authorized Signatory");
    }
  ...
  function gstBreakup(total) {
    ...
    // If not a GST invoice, taxable is same as total
  ...
  function invoiceText(inv, items, index) {
    ...
    if (inv.warranty_terms) {
      lines.push("");
      lines.push("Warranty & Coverage Terms:");
      lines.push(inv.warranty_terms);
    }
  ...
  async function handleEditInvoiceSubmit(e) {
    ...
    const { error } = await supabase.from("invoices").update({
      ...
      warranty_terms: editInvoiceForm.warranty_terms,
      customer_address: editInvoiceForm.customer_address,
  ...
  function printInvoice(inv, items, index) {
    ...
    `).join("");
    const premiumGstRows = inv.is_gst_invoice ? `
        ...
        <body>
          ...
          <div class="invoice-wrap">
            ...
            <div class="header">
              <div class="brand-line">
                ...
                <div>
                  ...
                  ${inv.is_gst_invoice ? `<div class="muted"><strong>GSTIN:</strong> ${business.gst_number || ""}</div>` : ""}
                </div>
              </div>
              <div class="invoice-title">
                <h2>Invoice</h2>
                <div><strong>No:</strong> ${invoiceNo}</div>
                <div><strong>Date:</strong> ${inv.created_at ? new Date(inv.created_at).toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" }) : ""}</div>
            ...
            <div class="box avoid-break" style="margin-top: 20px;">
              <div class="section-title">Warranty & Coverage Terms</div>
              <div style="white-space: pre-wrap; font-size: 13px; color: #374151;">${inv.warranty_terms}</div>
            </div>
            ` : ""}
            ...
            <div class="signature avoid-break">
              ...
              <div style="text-align: center;">
                ${business.signature_url ? `<img src="${business.signature_url}" alt="Authorized Signature" style="max-height: 52px; object-fit: contain; margin-bottom: 4px; display: block; margin-left: auto; margin-right: auto;" />` : `<div style="height: 48px;"></div>`}
    ...
    const gstRows = inv.is_gst_invoice ? `
      <tr><td>Taxable Value</td><td>₹${g.taxable.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</td></tr>
      <tr><td>CGST ${(g.rate / 2).toFixed(1)}%</td><td>₹${g.cgst.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</td></tr>
      <tr><td>SGST ${(g.rate / 2).toFixed(1)}%</td><td>₹${g.sgst.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</td></tr>
    ` : "";
          ...
          <div class="invoice-wrap">
            <div class="header">
              <div class="brand">
                ...
                ${inv.is_gst_invoice ? `<div><strong>GSTIN:</strong> ${business.gst_number || ""}</div>` : ""}
              </div>
              <div class="invoice-title">
                <h2>Invoice</h2>
                <div><strong>No:</strong> ${invoiceNo}</div>
    ...
    <>
              ...
              <div className="row-actions" onClick={(event) => event.stopPropagation()}>
                ...
                <button className="ghost-btn small" onClick={() => {
                  ...
                  setEditInvoiceForm({
                    ...
                    customer_address: inv.customer_address || bookingObj?.address || customerObj?.address || "",
                  });
                }}>Edit</button>
          ...
          <div className="panel" style={{ width: "100%", maxWidth: "500px", background: "#fff", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <form onSubmit={handleEditInvoiceSubmit} className="form-stack mt-md">
              ...
              <div style={{ marginTop: "14px" }}>
                <label>Warranty & Coverage Terms</label>
                <textarea
                  value={editInvoiceForm.warranty_terms}
                  onChange={e => setEditInvoiceForm({...editInvoiceForm, warranty_terms: e.target.value})}
                  rows={3}
                  style={{ width: "100%", padding: "8px", border: "1px solid #e5e7eb", borderRadius: "6px" }}
                  placeholder="e.g. 1 year parts & service coverage"
                />
              </div>
              <div style={{ marginTop: "14px" }}>
                <label>Customer Address</label>
                <textarea
                  value={editInvoiceForm.customer_address}
                  onChange={e => setEditInvoiceForm({...editInvoiceForm, customer_address: e.target.value})}
                  rows={2}
                  style={{ width: "100%", padding: "8px", border: "1px solid #e5e7eb", borderRadius: "6px" }}
      ...
      <DetailDrawer
        ...
        fields={detailInvoice ? [
          ...
          { label: "Created", value: detailInvoice.created_at ? new Date(detailInvoice.created_at).toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" }) : "" },
